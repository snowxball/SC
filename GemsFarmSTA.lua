...
local function getClosestFuelPosition(currentPos)
    ...
    local generatorLoc = getGeneratorPosition()  -- ambil lokasi generator sekali di awal
    ...
    while not foundValidFuel do
        ...
        local distToGenerator = math.huge
        if generatorLoc then
            distToGenerator = (targetPosition - generatorLoc).Magnitude
        end

        -- Hanya ambil fuel yang:
        -- 1. Tingginya tidak jauh (≤ 2)
        -- 2. Jaraknya ke generator ≥ 50
        if heightDifference <= 2 and distToGenerator >= 50 then
            foundValidFuel = bestTarget
        else
            -- Exclude fuel yang bermasalah
            if heightDifference > 2 then
                print("[-] Fuel anomaly detected at height: " .. tostring(targetPosition.Y) .. ". Excluding item.")
            end
            if distToGenerator < 50 then
                print("[-] Fuel terlalu dekat dengan generator (" .. string.format("%.1f", distToGenerator) .. " stud). Excluding item.")
            end
...
local function runPipeline()
    ...
    -- STEP 1-9: Kirim 8 fuel ke generator
    for i = 1, 9 do
